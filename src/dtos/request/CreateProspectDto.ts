...
export class CreateProspectDto {
  ...
  constructor(id: number | string, logo: string, name: (string | ((control: AbstractControl) => (ValidationErrors | null)) | undefined)[], sigle: string, capital: number, headOffice: string, legalRepresentative: string, yearOfCreation: number, dateOfRegistration: string, email: string, phone: string, fax: string, whatsapp: string, website: string, linkedin: string, ice: string, rc: string, ifm: string, patent: string, cnss: string, certificationText: string, businessDescription: string, legalStatus: LegalStatusDto | null, city: CityResponseDto | null, court: CourtResponseDto | null, companySize: CompanySizeResponseDto | null, industry: IndustryResponseDto | null, country: CountryResponseDto | null, proprietaryStructure: ProprietaryStructureDto | null, title: TitleResponseDto | null, reprosentaveJobTitle: JobTitleResponseDto | null) {
    this.id = id ? id : "";
    this.logo = logo;
    this.name = name;
    this.sigle = sigle;
