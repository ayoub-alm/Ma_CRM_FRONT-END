...
export class WmsAssetComponent implements OnInit, AfterViewInit {
  ...
  setupFilterPredicate(): void {
    this.storageCreditNotes.filterPredicate = (data: StorageCreditNoteResponseDto, filter: string) => {
      const searchTerms = filter.toLowerCase();
      const ref = (data.number || '').toLowerCase();
      const customer = (data.storageInvoice?.storageContract?.customer?.name || '').toLowerCase();
      const invoice = (data.storageInvoice?.number || '').toLowerCase();
  ...
  showBLDetails(row: any): void {
    this.router.navigateByUrl(`/admin/crm/wms/assets/show/${row.id}`).then()
  }

  applyFilter(event: Event): void {
    this.storageCreditNotes.filter = (event.target as HTMLInputElement).value.trim().toLowerCase();
    if (this.storageCreditNotes.paginator) {
      this.storageCreditNotes.paginator.firstPage();
    }
  }
